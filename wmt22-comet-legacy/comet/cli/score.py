...
from ..models import available_metrics, load_from_checkpoint
...
def score_command() -> None:
    ...
    if cfg.gpus > 1 and cfg.accelerator == "ddp":
        ...
        seg_scores = outputs.score
        std_scores = None if 'std' not in outputs else outputs.std
        torch.distributed.all_gather_object(gather_mean, seg_scores)
        if cfg.mc_dropout > 0:
        ...
        for i in range(len(cfg.translations)):
            ...
            if cfg.mc_dropout > 0:
                seg_scores.append(outputs.score)
                std_scores.append(outputs.std)
            else:
                seg_scores.append(outputs.score)
                std_scores.append(None)

            sys_scores.append(sum(outputs.score) / len(outputs.score))
        data = new_data
