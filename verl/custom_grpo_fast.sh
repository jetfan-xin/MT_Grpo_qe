...
# NCCL debug信息显示设置
export NCCL_DEBUG=WARN # or INFO
export NCCL_DEBUG_SUBSYS=GRAPH,COLL

# NCCL 等各种通信设置（保证先pin通）
# 尽量先别禁用 NCCL 能力（之前这些容易把通信逼到慢路径，导致更容易卡在捕获点）
# unset NCCL_P2P_DISABLE
unset NCCL_SHM_DISABLE
# unset NCCL_IB_DISABLE
export NCCL_P2P_DISABLE=1 # 避免复杂直连/跨架构 peer 访问引发的奇怪等待读写 【必须有】
export NCCL_IB_DISABLE=1 # 或者完全走 SHM（本机）禁 IB（如果没有 IB，就禁用）【必须有】
...
########## 2) 训练超参 ##########
# 修改了：actor_rollout_ref.rollout.enforce_eager: False -> True，否则无法运行
